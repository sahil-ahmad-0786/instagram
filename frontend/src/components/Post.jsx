...
const Post = ({ post }) => {
  ...
  const deletePostHandler = async () => {
    try {
      const res = await axios.delete(
        `https://instagram-20.vercel.app/api/v1/post/delete/${post?._id}`,
  ...
  const likeOrDislikeHandler = async () => {
    try {
      ...
      const res = await axios.get(
        `https://instagram-20.vercel.app/api/v1/post/${post._id}/${action}`,
        { withCredentials: true }
      );
  ...
  const commentHandler = async () => {
    try {
      const res = await axios.post(
        `https://instagram-20.vercel.app/api/v1/post/${post._id}/comment`,
        { text },
        {
          headers: {
  ...
  const bookmarkHandler = async () => {
    try {
      const res = await axios.get(
        `https://instagram-20.vercel.app/api/v1/post/${post?._id}/bookmark`,
        { withCredentials: true }
      );
      if (res.data.success) {
        toast.success(res.data.message);
